...
void board::position(std::string FEN){
    ...
    while ((j<=64) && (i<=splitted_fen[0].length())){
        ...
        switch (letter)
        {
            case 'p' : board_representation[square] = -1; pawn(-1,square); break;
            case 'n' : board_representation[square] = -2; knight(-1,square); break;
            case 'b' : board_representation[square] = -3; bishop(-1,square); break;
            case 'r' : board_representation[square] = -4; rook(-1,square); break;
            case 'q' : board_representation[square] = -5; queen(-1,square); break;
            case 'k' : board_representation[square] = -6; king(-1,square); break;
            case 'P' : board_representation[square] = 1; pawn(1,square); break;
            case 'N' : board_representation[square] = 2; knight(1,square); break;
            case 'B' : board_representation[square] = 3; bishop(1,square); break;
            case 'R' : board_representation[square] = 4; rook(1,square); break;
